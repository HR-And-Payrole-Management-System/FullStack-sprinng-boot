...
export default function OnboardingTemplateFormModal({ show, onHide, onSaved }) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [error, setError] = useState('');
  const [tasks, setTasks] = useState([
    { title: '', description: '', assignedRole: 'HR', dueOffsetDays: 0, mandatory: true, sequenceOrder: 1 },
  ]);
  ...
  const addTask = () => {
    setTasks((prev) => [...prev, {
      title: '', description: '', assignedRole: 'HR', dueOffsetDays: 0, mandatory: true, sequenceOrder: prev.length + 1,
    }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await onboardingTemplateApi.create({ name, description, tasks });
      onSaved();
      onHide();
    } catch (err) {
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton><Modal.Title>New Onboarding Template</Modal.Title></Modal.Header>
      <Modal.Body>
        {error && <div className="text-danger small mb-2">{error}</div>}
        <Row className="g-3 mb-3">
          
          <Col md={6}><Form.Group><Form.Label>Template Name</Form.Label>
            <Form.Control value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Standard Employee Onboarding" />
          </Form.Group></Col>
        ...
        <div className="fw-semibold small mb-2">Tasks</div>
        {tasks.map((t, idx) => (
          <Row key={idx} className="g-2 mb-2 align-items-end">
            ...
            <Col md={2}>
              <Form.Control type="number" placeholder="Due (days from start)" value={t.dueOffsetDays}
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" onClick={handleSubmit} disabled={saving || !name || tasks.some((t) => !t.title)}>
