...
import EmployeeLanyardCard from '../../components/idcard/EmployeeLanyardCard';

const PRINTABLE_STATUSES = ['ACTIVE', 'REISSUED'];

function IdCardLanyardPrint() {
  const { id } = useParams();
  ...
  return (
    <div className="min-h-full bg-slate-50 dark:bg-slate-900/60 px-6 py-6">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Print Lanyard Card</h1>
      ...
      <div className="flex justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 py-10 shadow-[0_1px_3px_rgba(15,23,42,0.06)]">
        <EmployeeLanyardCard employee={employee} card={card} companyLogo={companyLogo} />
      </div>
    </div>
  );
}

export default IdCardLanyardPrint;